...
public class PizzeriaDAO {
	...
	public Utente login(String username, String password) {
		...
		try {
			Utente u;
			u = em.createQuery("SELECT u FROM Utente u WHERE u.username=:u AND u.password=:p", Utente.class)
	...
	public List<Pizza> findAllbyUtenteId(int utenteId) {
		List<Pizza> pizze;
		EntityManager em = JPAUtil.getEntityManagerFactory().createEntityManager();
		TypedQuery<Pizza> q = em.createQuery("SELECT DISTINCT p FROM Pizza p " + "LEFT JOIN FETCH p.ingredienti "
				+ "JOIN FETCH p.impasto " + "WHERE p.utente.id = :id", Pizza.class);
